...
# newfilenames = [f for f in filenames if '.hpp' not in f else f.replace('hpp', 'h')]
newfilenames = [f if '.hpp' not in f else f.replace('hpp', 'h') for f in filenames]
print(newfilenames)
...
#    end and appending "ay" to the end. For example, python ends up as
#    ythonpay.
def pig_latin(text):
    say = " "
    ...
    for index, word in enumerate(words):
        ...
        words[index] = word[1:25] + word[0] + "ay"
...
#    others. Each fo the three values can be expressed as a octal number
#    summing each permission, with 4 corresponding to read, 2 to write, and
#    1 to execute. Or it can be written with a string using the letters, r, w, and x
...
#    For example, group_list("g", ["a", "b", "c"]) returns "g: a, b, c". Fill in the
#    gaps in this function to do that.

def group_list(group, users):
    members = f"{group}: " + ", ".join(users)
    return members
...
#    old and works as ___." for each one. For example, guest_list(('Ken', 30,
#    "Chef"), ("Pat", 35, "Lawyer"), ("Amanda", 25, "Engineer")) should print out:
#    Ken is 30 years old and works as Chef. Pat is 35 years old and works as 
#    Lawyer. Amanda is 25 years old and works as Engineer. Fill in the gaps in
#    this function to do that.

def guest_list(guests):
    for guest in guests:
        print("{} is {} years old and works as {}".format(*guest))
